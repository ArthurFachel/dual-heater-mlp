...
class _CIFARBasicBlock(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        return self.output_tracker(output)


class _CIFARResNet18(nn.Module):
    """ResNet-18 with a 3x3 CIFAR stem and GroupNorm."""
    ...
    def __init__(
        ...
        *,
        ...
        def convolution(*args, **kwargs):
            if slowheat:
                return SlowHeatConv2d(*args, **kwargs, **common)
        ...
        def tracker(channels: int) -> nn.Module:
            if slowheat:
                return SlowHeatChannelTracker(channels, **common)
            return nn.Identity()
    ...
    def get_slow_connections(
        ...
        for stage_index, stage in enumerate(self.stages):
            for block_index, block in enumerate(stage):
                ...
                if block.downsample_conv is not None:
                    connections.append(
                        (block.downsample_conv, source, 1, source_key)
                    )
    ...
    def get_slow_channel_modules(
        ...
        """Return affine normalizers and the channel state that protects them."""
